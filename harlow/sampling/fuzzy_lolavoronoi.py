...
class FuzzyLolaVoronoi(Sampler):
    ...
    def _fit_models(self):
        ...
        for i, dim_surrogate_model in enumerate(self.surrogate_models):
            dim_surrogate_model.fit(self.fit_points_x, self.fit_points_y[:, i])
    ...
    def _update_models(
        ...
        # Standard case assumes single model
        for i, dim_surrogate_model in enumerate(self.surrogate_models):
            dim_surrogate_model.update(new_fit_points_x, new_fit_points_y[:, i])
    ...
    def _predict(self):
        ...
        for i, dim_surrogate_model in enumerate(self.surrogate_models):
            y[:, i] = dim_surrogate_model.predict(self.test_points_x)
        return y
