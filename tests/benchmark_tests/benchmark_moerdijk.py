...
def offload_moerdijk():
    pass

def create_test_set(target_function, min_domain, max_domain, n):
...
# # ====================================================================
def create_train_test_sets(moerdijk_low, moerdijk_up, N_initial, N_test):
    # # Each column of train_Y corresponds to one GP
    domain_lower_bound = moerdijk_low
    domain_upper_bound = moerdijk_up
    target_func = offload_moerdijk
    print(f"Create training set N = {N_initial}:")
    initial_X, initial_y = create_test_set(
        target_func, domain_lower_bound, domain_upper_bound, N_initial
    )
    # To check surrogate updating
    print(f"Create update set N = {N_test}:")
    test_X, test_y = create_test_set(
        target_func, domain_lower_bound, domain_upper_bound, N_test
    )
    return initial_X, initial_y, test_X, test_y, target_func, domain_lower_bound, domain_upper_bound
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=50,
        type=int,
        help="The size of the initial set",
    )
    parser.add_argument(
        "--testset",
        type=str,
        help="path to pickle file containing test set",
    )
    parser.add_argument(
        "-i",
        "--iterations",
