...
def plot_function_custom(f, X, y = None, plot_sample_locations = False, plot_name = None, show = True, save = False, color = 'b'):
    ...
    if len(X.shape) == 1:
        ...
        ax = fig.gca(projection='3d')
        surface = ax.plot_trisurf(X[:,0], X[:,1], Z, cmap=cm.coolwarm, label=f.__name__ if plot_name is None else plot_name)
        fig.colorbar(surface)
